...
The `Controls` channel can be populated by hand (`Controls.Float("Sun yaw", ...)`), but most live-tunable values are *declared at their definition* and surfaced automatically — no per-variable field + dial + UBO-pack boilerplate:

- **`//@tune` (GLSL).** A uniform carries a decorator comment: `uniform float uSunIntensity; //@tune 0..16 = 9.42` (or `//@tune enum{Reinhard,ACES,AgX}`). `ShaderTunables.Scan(glsl)` (`Blix.Graphics`) parses these into `ShaderTunable` records (name, block, range, default, enum names) — read from the shader *source*, since comments are stripped from the compiled SPIR-V. `ShaderTunablePanel` (`Blix.Diagnostics`) seeds live values from the tag defaults, builds the overlay controls, and writes the tuned values into the per-frame UBO by name (reflection lands each at its std140 offset). CPU-side readers pull a value with `panel.Value("uSunIntensity")`.
- **`[Tune]` (C#).** A field on a settings object carries `[Tune]` (bool/enum) or `[Tune(min, max)]` (numeric): `[Tune(0f, 0.5f)] public float Density = 0.018f;`. `TuneReflection.Reflect(obj)` (`Blix.Diagnostics`) reflects these into `TunableField`s; `ObjectTunables(params object[])` groups them by type (type name minus "Settings") and dispatches Toggle/Enum/Float. The field is the source of truth — the game reads `fog.Density` directly and the overlay reflects + edits it live.

Both are strictly live inspect-and-edit; nothing is persisted and there's no rules/config/cook layer behind them. Click-to-pick selection (`IDebugSelectable` above) scopes a third kind — per-object values like a primitive's LOD error margin, edited for the current selection only. Verified by `Blix.Test.Graphics` Sections Q (GLSL scanner), R (`ShaderTunablePanel`), and S (`[Tune]` reflection).
...
### Asset-pipeline primitives (engine)

Above the cooked formats sit three composable engine primitives — the load / read / bundle half of the pipeline. They're library pieces the game *calls*, not a loader that runs the scene for it (see [architecture.md → Library, not framework](architecture.md)); `VulkanSponza` is their first client.

- **`MeshBundler`** (`Blix.Render`) — packs N source primitives into one shared vertex buffer + per-index-width (u16/u32) index buffers, returning a `BundledMesh` sub-range (`BaseVertex` + per-LOD `firstIndex`) for each. Indices stay primitive-local; `vkCmdDrawIndexed`'s `vertexOffset` rebases them, so u16 indices keep working though the buffer spans millions of verts. Pure-CPU `Pack` (unit-testable, no device) + `Bundle` (creates the GPU buffers). Order-preserving, so a caller that pre-sorts by (pipeline, material) gets contiguous draw groups. No material/pipeline knowledge — that's the game's. This is the consolidation in [Draw submission](#draw-submission--overdraw), factored out of the demo.
- **`AsyncLoadQueue<T>`** (`Blix.Render`) — the off-thread / CPU twin of `ResourceUploader`. Runs a producer (e.g. a parallel glTF parse) off-thread; once it completes, drains its items on the calling thread within a per-frame time budget (always ≥1, so no starvation), reporting fully-loaded when the queue empties. Generic, no graphics dependency. The game says *what* to parse and drives the drain.
- **`GltfTextureLoader`** (`Blix`, beside `GltfSceneInstance`) — resolves a `GltfMaterial`'s five channel textures to GPU handles (`MaterialTextures`), deduped + streamed. Owns the per-slot sRGB-vs-linear policy (glTF spec), cooked-`.blixtex` (streamed mip chain) vs decoded-PNG routing, per-source dedup, the standard glTF default fallbacks (white / flat-normal / black / neutral), and a budgeted `ResourceUploader`. Returns handles; the caller writes its own material UBO + descriptor binding (the UBO layout is the game's, post SPIR-V reflection).

`ResourceUploader` (`Blix.Render`) is the shared GPU-upload pump underneath: it allocates a texture's mip chain immediately (so a material binds a stable handle at once) and streams the per-mip uploads over the next frames within a budget, reading each mip lazily via a `Func<int,byte[]>` (one file open per `.blixtex`, not per mip). Drive `Drain(budgetMs)` per frame; `PendingCount == 0` means every mip has landed.

### Outstanding limits

- No caching at the `AssetDatabase` layer — every `Load` re-imports.
- Cooked binary formats (`.blixtex` / `.blixprobe` / `.blixmesh`) are a **separate** fast path — the `blix-cook` tool writes them and the engine streaming primitives above (`GltfTextureLoader` / `AsyncLoadQueue` / `MeshBundler`) load them — not wired through `AssetDatabase`, so the manifest path still parses source files.
- No hot reload.
...
Resolution-, FOV-, and primitive-size-aware; nearest-point (not centre) distance keeps a surface you stand on detailed where it matters. The same level is chosen across the lit, depth-pre-pass, and shadow passes so depth stays invariant. Selection lives in the game's draw loop (VulkanSponza's `PickLod`) by design — LOD/cull *policy* is the game's; the engine only bundles the geometry (`MeshBundler`) and carries each level's baked error. See [architecture.md → Library, not framework](architecture.md).
