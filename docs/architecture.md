...
**Matrices.** Column-vector convention everywhere: `projection * view * model * position`. CPU-side matrices are `System.Numerics.Matrix4x4`, but they're treated as column-vector matrices by the engine — `GraphicsMatrices` builds them in that form, and the Vulkan backend uploads them untransposed.
