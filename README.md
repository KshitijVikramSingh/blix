...
Every type has named fields and a deliberate-limits list. Concrete `Camera3D` / `DirectionalLight` / `GameObject` instead of entity-component soup. Two host adapters — `Blix.Runtime.OpenTK` (OpenGL) and `Blix.Runtime.Silk` (Vulkan, the active target) — on top of a typed graphics-command layer. PBR-grade rendering underneath — HDR + IBL, cascade and cube shadow maps, SSR, volumetric/froxel fog, bloom, ACES/AgX tonemapping, GPU-driven indirect draw with screen-space-error LOD — kinematic physics + collision, glTF skeletal animation, OpenAL positional audio, sprite/text UI.
