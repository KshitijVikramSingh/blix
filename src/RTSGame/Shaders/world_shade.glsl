// <b>The world's shading, shared by the plain and the skinned fragment stages.</b>
//
// Extracted rather than duplicated: this is three hundred lines of sun, shadow cascades, hemispheric
// ambient, aerial perspective and the fog-of-war veil, and a body that went through a second copy of it
// would drift from everything else in the settlement within a session. §170.
//
// The includer supplies rts_surface_tint(), which is the ONE thing the two stages disagree about: the plain
// stage hands back the instance tint as-is, and the skinned stage multiplies it by the character's albedo
// texture. Everything below is identical for both.
//
// Requires world_varyings.glsl and a definition of rts_surface_tint() ahead of it.
...
void main() {
    ...
    // The one line the two fragment stages differ by — see rts_surface_tint in the includer.
    vec4 tint = rts_surface_tint();
    float surface = tint.a;
